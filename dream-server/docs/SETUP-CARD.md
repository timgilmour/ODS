...
`scripts/generate-setup-card.py` produces printable 4x6 setup cards for fulfillment: drop one in the box with each Dream Server unit so the recipient can scan two QR codes without typing anything. The default card opens the first-run wizard; `--mode factory-owner` prints the owner-card handoff that opens Dream Talk.

## What it generates

A portrait card with:

1. **Top band** - "DREAM SERVER" wordmark, the unit's mDNS name (e.g. `dream.local`), and a one-line tagline.
2. **Two QR codes side by side:**
   ...
   - **Right - OPEN SETUP or OPEN DREAM TALK.** In default setup mode this is a setup URL such as `http://192.168.7.1/setup`. In factory-owner mode this is the owner magic-link URL generated by Setup / Owner.
3. **Plain-text fallback block** - SSID, password, and URL printed verbatim for phones that will not scan the QR.
4. **Footer** - "DreamServer is open-source - light-heart-labs.com", plus an optional per-unit serial number.
...
This is an operator-side tool, not a runtime service, so the deps are not bundled with any Dream Server container.
...
python3 scripts/generate-setup-card.py \
  ...
  --ssid 'Dream-Setup-A4F2' \
  --password 'xxxxxxxx' \
  --owner-url 'http://auth.dream-a4f2.local/magic-link/OWNER_TOKEN' \
  --device-name 'dream-a4f2.local' \
  --serial 'DRM-2026-A4F2' \
...
| Flag | Required | Notes |
|---|---|---|
| `--mode` | no | `setup` by default; use `factory-owner` for Wi-Fi + owner Dream Talk QR cards. |
...
- Factory owner cards open the mobile Dream Talk portal. It is backed by Hermes, but it avoids the full advanced Hermes interface for the recipient.
