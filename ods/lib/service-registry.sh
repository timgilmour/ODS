...
sr_load() {
    ...
    # explicitly enabled ODS_SR_AUTO_INSTALL_PYYAML.
    if ! "$PYTHON_CMD" -c "import yaml" 2>/dev/null; then
        if [[ "${ODS_SR_AUTO_INSTALL_PYYAML:-}" == "1" ]] && declare -f pkg_install &>/dev/null && declare -f pkg_resolve &>/dev/null; then
...
#
# Users copy container names (e.g. `ods-token-spy`) from `docker ps` and
# expect them to work as arguments to `ods restart|stop|start|update`. The
# registry loader names every container `ods-<sid>` (or the manifest's
# explicit `container_name`, which by convention follows the same pattern),
# so stripping a leading `ods-` recovers the alias key when the literal
# input doesn't match an alias.
sr_resolve() {
    sr_load
    local input="$1"
    if [[ -z "${SERVICE_ALIASES[$input]:-}" && "$input" == ods-* ]]; then
        local _stripped="${input#ods-}"
...
sr_container() {
    ...
    local sid
    sid=$(sr_resolve "$1")
    echo "${SERVICE_CONTAINERS[$sid]:-ods-$sid}"
}
