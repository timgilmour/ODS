...
assert_contains "$bootstrap" 'is_commit_ref()' "bootstrap should distinguish immutable commit refs from branch refs"
assert_contains "$bootstrap" 'clone_args\+=\(--branch "\$ODS_REF"\)' "bootstrap branch ref override should apply to git clone"
assert_contains "$bootstrap" 'fetch --depth 1 origin "\$ref"' "bootstrap commit ref override should fetch the exact SHA"
assert_contains "$bootstrap" 'checkout --detach FETCH_HEAD' "bootstrap commit ref override should detach at the fetched SHA"
