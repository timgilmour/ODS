...
try {
  ...
  // Anyone with `docker logs ods-openclaw` access could otherwise harvest
...
if (token && fs.existsSync(HTML_PATH)) {
  try {
    ...
    // ODS is LAN-bound. With BIND_ADDRESS=0.0.0.0, anyone on the LAN
    ...
    if (AUTO_TOKEN_ALLOWED) {
      ...
      jsCode = [
        ...
        '// this unauthenticated static asset when ODS is LAN-bound.',
